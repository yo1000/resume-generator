...
export default function loadResumeData(config) {
    const tomlPath = path.resolve(config.data.location, config.data.resumeFileName);
    ...
    let age = calcAge(config.issueDate, birthdate)
    ...
    return {
        ...
        profile: {
            ...
            m: birthdate?.getMonth() ? birthdate?.getMonth() + 1 : "",
            d: birthdate?.getDate() ?? "",
            age: age
        },
        contact_primary: {
